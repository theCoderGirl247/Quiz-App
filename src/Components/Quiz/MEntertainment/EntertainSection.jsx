...
import { entertainmentQuiz } from './EntertainmentQB.jsx';
import { Hint } from "./EntertainmentQB.jsx";
...
function EntertainmentQuiz() {
    ...
    function checkAnswer(ourOption) {
        if (ourOption === entertainmentQuiz[currentQuestion].Answer) {
            setScore(s => s + 1);
        }
    ...
    function moveToNextQuestion() {
        let randomIndex;
        do {
            randomIndex = Math.floor(Math.random() * entertainmentQuiz.length);
    ...
    return (
        <div id={styles.MainQuiz}>
            <div className={styles.waveText}>
                {Array.from('Entertainment Quiz').map((char, index) => (
                    <span key={index} style={{ '--i': index }}>{char}</span>
                ))}
            </div>
            <div id={styles.quizBox}>
                <h3>Q {count + 1}. {entertainmentQuiz[currentQuestion].prompt}</h3>
                <button className={styles.optionButton} onClick={() => checkAnswer("optionA")}>A. {entertainmentQuiz[currentQuestion].optionA}</button>
                <button className={styles.optionButton} onClick={() => checkAnswer("optionB")}>B. {entertainmentQuiz[currentQuestion].optionB}</button>
                <button className={styles.optionButton} onClick={() => checkAnswer("optionC")}>C. {entertainmentQuiz[currentQuestion].optionC}</button>
                <button className={styles.optionButton} onClick={() => checkAnswer("optionD")}>D. {entertainmentQuiz[currentQuestion].optionD}</button>
